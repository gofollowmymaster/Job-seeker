...
主要有两种结构：**B+tree、hash和全文索引**

###### Hash索引：
Hash 索引结构的特殊性，其检索效率非常高，索引的检索可以一次定位，不像B-Tree 索引需要从根节点到枝节点，最后才能访问到页节点这样多次的IO访问，所以**Hash 索引的查询效率要远高于 B-Tree 索引**。

**原理**：哈希索引基于哈希表实现，只有精确匹配索引所有列的查询才会有效。对每一行数据，存储引擎都会对所有的索引列计算一个哈希码（hash code），哈希码是一个比较小的值，并且不同键值的行计算出来的哈希码不一样，哈希索引将所有的哈希码存储在索引中，同时在哈希表中保存指向数据行的指针。

**限制**：哈希索引数据并不是按照索引值顺序存储的，所以也就无法用于排序。

​	   哈希索引不支持部分索引列匹配查找，因为哈希索引始终是使用索引列的全部内容来计算哈希值。

​	   哈希索引只支持等值查找，包括（=、<=>、in（）），不支持范围查找。

​	   如果哈希冲突多，一些索引维护操作的代价会很大。

**哈希冲突**：不同索引列值却有相同的哈希值。

**创建自适应哈希索引**：优势---使用很小的索引就可以为超长的健创建索引。

   对值很长的列，进行hash计算，保存为单独的一列，利用该列进行查询。

###### B-Tree索引

是Mysql的关键字，但具体存储引擎有不同的存储结构，其中Innodb使用B+Tree。
...
[全文索引原理详解](https://blog.csdn.net/shb_derek1/article/details/77478691)
...
- 覆盖索引
  ...
  如果查询的列恰好是索引的一部分，那么查询只需要在索引文件上进行，不需要进行到磁盘中找数据，若果查询的列不是索引的一部分则要到磁盘中找数据
...
- **服务器优化**
  ...
  innodb_buffer_pool_size(配置innodb缓冲池的大小)：这是InnoDB最重要的设置，对InnoDB性能有决定性的影响。默认的设置只有8M，所以默认的数据库设置下面InnoDB性能很差。在只有InnoDB存储引擎的数据库服务器上面，可以设置60-80%的内存。更精确一点，在内存容量允许的情况下面设置比InnoDB tablespaces大10%的内存大小。
...
- **数据库优化**
  ...
  - 索引使用
    ...
    - 应尽量**避免**在 where 子句中对字段进行**null 值判断、使用 !=或<> 操作符、前缀%、对字段进行表达式及函数操作**，否则将导致引擎放弃使用索引而进行全表扫描。
    - 尽量选择区分度高的列作为索引。
